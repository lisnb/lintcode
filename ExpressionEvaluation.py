...
# -*- coding: utf-8 -*-
# @Author: LiSnB
# @Date:   2015-05-07 09:24:57
# @Last Modified by:   LiSnB
# @Last Modified time: 2015-05-07 12:41:38
...
class Solution:
    ...
    def evaluateExpression(self, expression):
        # write your code here
        expre = self.convertToRPN(expression)
        r = []
        for ex in expre:
            if ex not in self.operset:
                r.append(ex)
            else:
                right = r.pop()
                left = r.pop()
                r.append(self.calculate(left,right, ex))
        return r[0]

if __name__ == '__main__':
    s = Solution()
    expression = ["3", "-", "4", "+", "5"]
    print s.evaluateExpression(expression)